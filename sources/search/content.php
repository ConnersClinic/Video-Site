...
if ($pt->config->total_videos > 1000000) {
    ...
    // $get_videos = $db->rawQuery("SELECT * FROM " . T_VIDEOS . " WHERE MATCH (title) AGAINST ('$keyword')  $privacy ".$category.$date." ORDER BY id ASC LIMIT 20");

    // pagination system 
    // $get_videos = $db->where("title LIKE '%$keyword%'  $privacy  ".$category.$date)->orderBy('id', 'ASC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $get_videos = $db->where("MATCH (title) AGAINST ('$keyword')  $privacy  ".$category.$date)->where('approved',1)
    //->orderBy('id', 'ASC')
    ->orderBy("(CASE WHEN title LIKE '%$keyword%' THEN 1 ELSE 2 END), id", 'ASC')
    ->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system
} else {
    //$get_videos = $db->rawQuery("SELECT * FROM " . T_VIDEOS . " WHERE title LIKE '%$keyword%'  $privacy ".$category.$date." ORDER BY id ASC LIMIT 20");
    
    // pagination system 
    $get_videos = $db->where("(title LIKE '%$keyword%' OR tags LIKE '%$keyword%' OR description LIKE '%$keyword%')  $privacy  ".$category.$date)->where('approved',1)
    //->orderBy('id', 'ASC')
    ->orderBy("(CASE 
                   WHEN title LIKE '%$keyword%' THEN 1 
                   WHEN description LIKE '%$keyword%' THEN 2 
                   WHEN tags LIKE '%$keyword%' THEN 3 
                   ELSE 4 
               END), id", 'ASC')
               ->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
