...
// $home_top_videos = $db->where('privacy', $privacy,'IN')->orderby('views', 'DESC')->get(T_VIDEOS, 6);
// $top_videos_html = '';
...
$video_obj = $db->where('featured', '1')->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_movie', 0)->where('privacy', $privacy,'IN')->where('live_time', 0)->where('approved',1)->where('is_short', 0)->orderBy('RAND()')->getOne(T_VIDEOS);
...
if (empty($get_video)) {
    ...
    $db->where('converted', '2','<>');
    $get_video = PT_GetVideoByID($db->where('privacy', $privacy,'IN')->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_movie', 0)->where('live_time', 0)->where('approved',1)->where('is_short', 0)->orderBy('id', 'DESC')->getOne(T_VIDEOS), 0, 1, 0);
}
