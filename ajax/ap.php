...
if (!isset($pt->categories[$category])) {
    echo json_encode([
        'status' => 400,
...
$db->where('id', $video_ids, 'IN');

if ($db->update('videos', ['category_id' => $category])) {
...
if ($db->update('videos', ['privacy' => $status])) {
    echo json_encode(['status' => 200]);
} else {
    echo json_encode([
        'status' => 500,
...
if ($first == 'transcribe_settings') {
    $allowed = array(
        ...
        'watch_cta2_headline',
        'watch_cta2_body',
        'watch_cta2_button_text',
        'watch_cta2_button_url',
    ...
    if (!empty($_POST)) {
        foreach ($allowed as $key) {
            ...
            if ($key == 'openai_api_key') {
                ...
                $val = $_POST[$key];
            } elseif (in_array($key, array('watch_cta_eyebrow', 'watch_cta_headline', 'watch_cta_button_text', 'watch_cta_button_url', 'watch_cta2_eyebrow', 'watch_cta2_headline', 'watch_cta2_button_text', 'watch_cta2_button_url'), true)) {
                $val = trim($_POST[$key]);
            } else {
